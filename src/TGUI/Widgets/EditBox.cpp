...
namespace tgui
{
    ...
    void EditBox::setText(const String& text)
    {
        ...
        if ((m_regexString == ".*") || std::regex_match(text.toAnsiString(), m_regex))
            m_text = std::u32string(text);
        else // Clear the text
            m_text = U"";
    ...
    void EditBox::setMaximumCharacters(unsigned int maxChars)
    {
        ...
        if ((m_maxChars > 0) && (m_displayedText.length() > m_maxChars))
        {
            ...
            m_displayedText.erase(m_maxChars, std::u32string::npos);
    ...
    bool EditBox::setInputValidator(const String& regex)
    {
        ...
        m_regexString = regex.toAnsiString();
        setText(m_text);
        return true;
    }
    ...
    void EditBox::textEntered(char32_t key)
    {
        ...
        if (m_regexString != ".*")
        {
            ...
            if (!std::regex_match(String(text).toAnsiString(), m_regex))
                return;
        }
